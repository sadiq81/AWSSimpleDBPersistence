...
{
	public class BatchDeleteAttributtesResponseUnMarshaller
	{
		protected XmlSerializer Serializer;

		public Response Response { get; set; }

		public BatchDeleteAttributtesResponseUnMarshaller (HttpResponseMessage message)
		{
			XDocument doc = XDocument.Load (message.Content.ReadAsStreamAsync ().Result);

			if (message.StatusCode.Equals (HttpStatusCode.OK)) {
				Serializer = new XmlSerializer (typeof(BatchDeleteAttributesResponse), Response.NameSpace);
				Response = (BatchDeleteAttributesResponse)Serializer.Deserialize (doc.CreateReader ());
